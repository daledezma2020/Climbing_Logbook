...
- **Images**: avatars are the only stored images. `POST /api/users/me/avatar` writes to local disk via `IAvatarStorage` / `LocalDiskAvatarStorage` (magic-byte type sniffing, 2 MB cap, `Storage:AvatarPath` config) and is served back from `/uploads/avatars`. `Climb.PictureUrl` and `Climb.VideoUrl` remain URL-only strings pointing at external hosts. Avatars resolve `AppUser.PictureUrl`, then the Auth0 `picture` claim, then initials. See the "Image storage" section in `readme.md`.
