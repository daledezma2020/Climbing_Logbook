...
No automated test project is configured yet. Before opening a PR, run `dotnet build`, `npm run lint`, and `npm run build`, then manually verify affected endpoints and browser workflows. Add backend tests in `api.Tests/`; add frontend tests beside source files as `*.test.tsx`.

## Commit & Pull Request Guidelines

Use short, imperative commit messages such as `chore: ...`, `fix: ...`, and `feat: ...`. Keep each commit focused.
